...
void Cytron_SmartDriveDuo::control(signed int motorLSpeed, signed int motorRSpeed)
{
  switch (_mode) {
    ...
    case SERIAL_PACKETIZED:
      ...
      commandByte = map(motorLSpeed, -100, 100, 0, 255); // -100~100を0~255に変換
      checksum = headerByte + addressByte + commandByte;
      if (hardwareSerial == true) {
        Serial.write(headerByte); // パケットの先頭を示す
        Serial.write(addressByte); // モータドライバのアドレス
        Serial.write(commandByte); // モータの回転速度(0~255) 0:逆 127:停止 255:正
        Serial.write(checksum); // チェックサム
      }
      else {
